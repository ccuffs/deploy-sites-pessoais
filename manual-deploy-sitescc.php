...
function setSiteAsUpdate($apiUrl, $id, $uid){

    $client = new Client(['base_uri' => $apiUrl]);

    $options = ['headers' => ['Accept' => 'application/hal+json', 'Content-Type' => 'application/hal+json'], 'body' => json_encode(["id"=>$id])];

    $response = $client->patch('/api/sites/updated', $options);

    $resp = json_decode($response->getBody());

    if (!$resp->success){
        echo magenta("  ERROR ({$resp->error}) ") . white('uid = ' . $uid . ', id = ' . $id) . PHP_EOL;
    }
}
...
$cli->description('Faz deploy dos sites pessoais do curso a partir de uma lista de usuários e fontes.')
    ...
    ->opt('api-url:a', 'Url para a API da intranet.');
...
$api_url = $args->getOpt('api-url', "https://cc.uffs.edu.br/intranet");
...
foreach($sites as $site) {
    ...
    if(!empty($site->source_url)) {
        ...
        setSiteAsUpdate($api_url, $site->id, $site->uid);
    } else {
